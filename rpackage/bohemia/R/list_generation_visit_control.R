...
#' @param data_list A list of dataframes as generated by retrieve_odkx_data
...
#' @param start_date Remove observations updated after this date (NULL means don't remove)
#' @import dplyr
#' @import PKI
#' @import readr
#' @import rmarkdown
...
list_generation_visit_control <- function(census_data,
  ...
  if(!is.null(location_code)){
    ...
    ref <- ref %>%
      filter(`group_location-hh_hamlet_code` %in% location_code)
    # Get enumerations
    country <- locs$Country[1]
    if(country == 'Mozambique'){
      enum <- agg_list$enumerationscensus %>%
        filter(!substr(`group_location-agregado`, 1, 3) %in% location_code)
    }
  ...
  if(fake_data){
    ...
    df <- census_data
    # save(df, start_date, data_list, file = '/tmp/joe.RData')
    # Remove the data that has already been collected in the minicensus
    if(!is.null(start_date)){
      # Identify households to be removed
      dlc <- data_list$census %>%
        mutate(date = as.Date(`_savepoint_timestamp`)) %>%
        filter(date >= start_date)
      # removing observations
      df <- df %>% filter(!hh_id %in% dlc$hh_id)
    }
  ...
  if(nrow(df) > 0){
    ...
    message('NO HOUSEHOLDS MEET CRITERIA. RETURNING NULL')
